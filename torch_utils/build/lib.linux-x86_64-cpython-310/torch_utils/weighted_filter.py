...
class WeightedFilterFunction(torch.autograd.Function):
	@staticmethod
	def forward(ctx, input, weights, kernel_size, splat, level):
		...
		return torch_utils_cpp.weighted_filter_forward(input, weights, kernel_size, splat, level)
...
class WeightedFilter(torch.nn.Module):
	...
	def forward(self, input, weight):
		bilat = WeightedFilterFunction.apply(input, weight, self.kernel_size, self.splat, self.level)
